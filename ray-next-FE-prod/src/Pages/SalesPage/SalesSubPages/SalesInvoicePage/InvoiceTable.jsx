...
const InvoiceTable = ({ formik, edit, setEdit, item, setItem }) => {
            ...
            {edit === index ? (
              <InventorySearch
                ...
                onChange={(e) => {
                  ...
                  updateData(index, "Item description", e.description);

                  updateData(index, "Rate", e?.unit_details?.rate);
                }}
              />
            ) : (
          ...
          <div className="maxw">
            {edit === index ? (
              <BorderdInput
                ...
                defaultValue={select?.unit_details?.rate || record["Rate"]}
        ...
        return (
          <div className="maxw">
            <p>
              {Number(
                record["Gross amount"] -
                  (Number(record["Discount %"]) / 100) * record["Gross amount"]
              )}
            </p>
          </div>
        );
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (
                      ...
                      if (
                        formik.values.sales_invoice_items?.filter(
                          (e) => e.index === index
                        ).length > 0
                      ) {
                        formik.setFieldValue("sales_invoice_items", [
                          ...formik.values.sales_invoice_items?.filter(
                            (e) => e.index !== index
                          ),
                          {
                            ...
                            gross_amount: Number(record["Gross amount"]),
                            discount_percentage: Number(record["Discount %"]),
                            discount_amount:
                              Number(record["Discount %"] / 100) *
                              Number(record["Gross amount"]),
                            net_amount: Number(
                              record["Gross amount"] -
                                (Number(record["Discount %"]) / 100) *
                                  record["Gross amount"]
                        ...
                        formik.setFieldValue("sales_invoice_items", [
                          ...formik.values.sales_invoice_items,
                          {
                            index: index,
                            item_code: record["item_code"],
                            description:
