...
import { useGetReturn } from "../../../../Queries/SalesQuery/SalesQuery";
import Pagination from "../../../../CommonComponents/OtherComponent/Pagination";
import dayjs from "dayjs";
const HistoryReturnTable = ({ tabIndex }) => {
  const [filter, setFilter] = useState({
    pageNo: 1,
    pageCount: 10,
  });
  const [index, setIndex] = useState();
  const { data, isLoading, refetch } = useGetReturn(filter);

  useEffect(() => {}, [tabIndex, data]);
  ...
  const invoiceData = data?.data?.data?.data?.map((e) => ({
    ...
    Discount:  e?.summary?.discount_amount !== undefined ? "$" + e?.summary?.discount_amount : 0  ,
    "Net Amt":
    ...
    //     : dayjs(e?.due_date).format("DD MMM YYYY"),
...
export default HistoryReturnTable;
