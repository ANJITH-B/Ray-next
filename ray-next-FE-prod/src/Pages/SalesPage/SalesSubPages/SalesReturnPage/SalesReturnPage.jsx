...
import {
  useAddSalesReturn,
  useGetInvoice,
} from "../../../../Queries/SalesQuery/SalesQuery";
...
const SalesReturnPage = () => {
  ...
  const { data, isLoading: invoiceLading } = useGetInvoice({
    pageNo: 1,
    pageCount: 20,
  });
  const invoiceID = data?.data?.data?.data?.map((e) => {
    return { value: e.invoice_id, label: e.invoice_id };
  });
  const initialValues = useCallback(
    {
      sales_return_id: id?.data?.data?.return_id,
  ...
  const totalAmount = formik?.values?.sales_return_items
    ?.map((e) => e.net_amount)
    .filter((e) => e !== undefined)
    .reduce((a, b) => a + b, 0);

  const { mutateAsync: addInvoice, isLoading } = useAddSalesReturn();
  const handleSubmit = () => {
    const data = {
      sales_return_id: formik.values.sales_return_id,
      ...
      summary: {
        ...
        net_amount: formik.values?.sales_return_items
        .map((e) => {
          return e.net_amount;
        })
        ?.reduce((e, i) => {
      ...
      sales_return_items: formik.values.sales_return_items,
    };
      ...
      <div className="p-8  max-w-[1512px]   h-auto m-auto">
        ...
        <div className="mb-12">
          <div className="mb-6 flex w-full items-center justify-between">
            ...
            >
              {formik.errors.sales_return_items ? (
                <div
                  className={`${
                    formik.errors.sales_return_items
                ...
                >
                  <Tooltip
                    trigger={"hover"}
                    title={formik.errors.sales_return_items}
                    color="white"
        ...
        <div className="flex items-center   max-w-[1512px]   h-full m-auto">
          ...
          <div className="flex-[1] flex justify-between">
            ...
            <div className="flex flex-[.6] flex-col gap-6">
              ...
              <div>
                <p className=" font-semibold  text-2xl 2xl:text-3x">
                  ${" "}
                  {(
                    totalAmount -
